/*global PDFJS, escape, unescape, error, console, prompt, FirefoxCom, Element, Object, object, Stats, Reconciler, PDFBug */
 ...
 var Cache = function cacheCache(size) {
   ...
   this.push = function cachePush(view) {
     ...
     if (i >= 0) { data.splice(i); }
     data.push(view);
     if (data.length > size) { data.shift().destroy(); }
   };
 };
 ...
 var ProgressBar = (function ProgressBarClosure() {
   ...
   ProgressBar.prototype = {
     ...
     updateBar: function ProgressBar_updateBar() {
       ...
       if (this._percent > 95) { this.div.classList.add('full'); }
     ...
     set percent(value) {
       this._percent = clamp(value, 0, 100);
       this.updateBar();
     }
   };
 ...
 var PDFView = {
   ...
   parseScale: function pdfViewParseScale(value, resetAutoSettings, noScroll) {
     ...
     var pageWidthScale = (container.clientWidth - kScrollbarPadding) / currentPage.width * currentPage.scale / kCssUnits;
     var pageHeightScale = (container.clientHeight - kScrollbarPadding) / currentPage.height * currentPage.scale / kCssUnits;
   ...
   set page(val) {
     var pages = this.pages,
         input = document.getElementById('pageNumber'),
   ...
   get supportsPrinting() {
     var canvas = document.createElement('canvas'),
         value = 'mozPrintCallback' in canvas;
     // shadow
       ...
       function getDocumentError(message, exception) {
         if (exception && exception.name === 'PasswordException') {
           if (exception.code === 'needpassword') {
             var promptString = mozL10n.get('request_password', null, 'PDF is protected by a password:');
         ...
         loadingIndicator.textContent = mozL10n.get('loading_error_indicator', null, 'Error');
         var moreInfo = { message: message };
         self.error(mozL10n.get('loading_error', null, 'An error occurred while loading the PDF.'), moreInfo);
         self.loading = false;
       },
   ...
   error: function pdfViewError(message, moreInfo) {
     var moreInfoText = mozL10n.get('error_build', {build: PDFJS.build}, 'PDF.JS Build: {{build}}') + '\n';
     if (moreInfo) {
   ...
   renderHighestPriority: function pdfViewRenderHighestPriority() {
     ...
     var visiblePages = this.getVisiblePages(),
         pageView = this.getHighestPriority(visiblePages, this.pages, this.pageViewScroll.down);
     ...
     if (this.sidebarOpen) {
       ...
       if (thumbView) { this.renderView(thumbView, 'thumbnail'); }
     }
   },
   ...
   search: function pdfViewStartSearch() {
     ...
     if (!pageFound) {
       ...
       noResults.textContent = mozL10n.get('search_terms_not_found', null, '(Not found)');
     ...
     if (hash.indexOf('=') >= 0) {
       ...
       if ('page' in params) {
         ...
         if ('zoom' in params) {
           ...
           if (zoomArgNumber) { zoomArg = zoomArgNumber / 100; }

           var dest = [null, {name: 'XYZ'}, (zoomArgs[1] || 0), (zoomArgs[2] || 0), zoomArg];
           var currentPage = this.pages[pageNumber - 1];
           currentPage.scrollIntoView(dest);
         } else {
   ...
   getVisibleElements: function pdfViewGetVisibleElements(scrollEl, views, sortByVisibility) {
     ...
     for (; i <= ii && currentHeight < bottom; ++i) {
       ...
       hidden = Math.max(0, top - currentHeight) + Math.max(0, nextHeight - bottom);
       percent = Math.floor((viewHeight - hidden) * 100.0 / viewHeight);
       visible.push({ id: view.id, y: currentHeight, view: view, percent: percent });
   ...
   function setupAnnotations(pdfPage, viewport) {
     function bindLink(link, dest) {
       ...
       link.onclick = function pageViewSetupLinksOnclick() {
         if (dest) { PDFView.navigateTo(dest); }
         return false;
       };
     }
     ...
     function createCommentAnnotation(type, item) {
       ...
       image.alt = mozL10n.get('text_annotation_type', {type: type}, '[{{type}} Annotation]');
       ...
       for (var i = 0; i < items.length; i++) {
         ...
         switch (item.type) {
           case 'Link':
             ...
             link.href = item.url || '';
             if (!item.url) { bindLink(link, ('dest' in item) ? item.dest : null); }
             div.appendChild(link);
             break;
           case 'Text':
             var comment = createCommentAnnotation(item.name, item);
             if (comment) { div.appendChild(comment); }
             break;
           case 'Widget':
             // TODO: support forms
             PDFView.fallback();
 ...
 var DocumentOutlineView = function documentOutlineView(outline) {
   ...
   if (!outline) {
     ...
     noOutline.textContent = mozL10n.get('no_outline', null, 'No Outline Available');
     ...
     function renderTextLayer() {
       ...
       if (textDiv.dataset.textLength > 0) {
         ...
         if (textDiv.dataset.textLength > 1) { // avoid div by zero
           ...
           CustomStyle.setProp('transform' , textDiv, 'scale(' + textScale + ', 1)');
 ...
 (function fullscreenClosure() {
   function fullscreenChange(e) {
     ...
     if (!isFullscreen) { PDFView.exitFullscreen(); }
   }
